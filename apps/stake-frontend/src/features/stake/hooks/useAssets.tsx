    ...
    queryFn: async () => {
      ...
      return userAssets.items
        .map((asset) => {
          ...
          return new PublicKey(Uint8Array.from(asset.address)).toBase58();
        })
        .filter((asset) => asset !== null) as string[];
    },
...
export const useAsset = (assetId: string | undefined) => {
  ...
  const { data: stakeRecord } = useStakeRecord(assetId);

  const query = useQuery({
    queryKey: ["useAsset", assetId, stakeRecord],
    queryFn: async () => {
      if (!assetId || !zkNftProgram || stakeRecord === undefined) return null;
      ...
      const assetAccount = await rpc.getCompressedAccount(
        bn(new PublicKey(assetId).toBuffer())
      );
      if (!assetAccount?.data) {
        throw new Error("Failed to get asset account");
      }
    ...
    enabled: !!assetId && !!zkNftProgram && stakeRecord !== undefined,
  });
